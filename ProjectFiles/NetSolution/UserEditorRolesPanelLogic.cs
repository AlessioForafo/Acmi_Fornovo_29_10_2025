...
public class UserEditorRolesPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIRoles()
    {
        ...
        foreach (var role in roles.Children)
        {
            ...
            if (editable.Value)
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, Acmi_Fornovo_29_10_2025.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasRole(role.NodeId))
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, Acmi_Fornovo_29_10_2025.ObjectTypes.GroupLabel);
            }
