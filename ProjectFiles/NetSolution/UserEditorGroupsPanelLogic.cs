...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            ...
            if (editable.Value)
            {
                groupUiObject = InformationModel.MakeObject<Panel>(group.BrowseName, Acmi_Fornovo_29_10_2025.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasGroup(group.NodeId))
            {
                groupUiObject = InformationModel.MakeObject<Panel>(group.BrowseName, Acmi_Fornovo_29_10_2025.ObjectTypes.GroupLabel);
            }
